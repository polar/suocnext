class UserNotifier < ActionMailer::Base
  ...
  include BaseHelper
  ActionMailer::Base.default_url_options[:host] = configatron.sub('http://', '')

  def signup_invitation(email, user, message)
    setup_sender_info
    @recipients  = "#{email}"
    @subject     = "#{user.login} would like you to join #{configatron.community_name}!"
    @sent_on     = Time.now
    @body[:user] = user
  ...
  def comment_notice(comment)
    ...
    @subject     += "#{comment.username} has something to say to you on #{configatron.community_name}!"
  ...
  def signup_notification(user)
    setup_email(user)
    @subject    += "Please activate your new #{configatron.community_name} account"
  ...
  def post_recommendation(name, email, post, message = nil, current_user = nil)
    ...
    @sent_on     = Time.now
    setup_sender_info
    @subject     = "Check out this story on #{configatron.community_name}"
  ...
  def activation(user)
    setup_email(user)
    @subject    += "Your #{configatron.community_name} account has been activated!"
    @body[:url]  = "#{APP_URL}"
  end
  
  def reset_password(user)
    setup_email(user)
    @subject    += "#{configatron.community_name} User information"
  end
