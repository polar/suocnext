namespace :db do
  ...
  task :backup do
    msg "Backing up #{Rails.env} database"
    archive = backup
  ...
  task :stage do
    ...
    if Rails.env == "staging"
      ...
      restore_db(archive)
      dirsymlink(File.join(Rails.root,"public"),
                 File.join(ENV["PRODUCTION_PATH"],"public"),
                 [File.join(Rails.root,"public","plugin_assets")])
    else
  ...
  def backup(db = Rails.env)
    archive = "/tmp/#{archive_name(Rails.env,'db')}"
    adapter, database, user, password = retrieve_db_info
    if adapter == "mysql"
  ...
  def retrieve_db_info
    ...
    msg  "Retrieving DB INFO from #{Rails.root}/config/database.yml for #{Rails.env}"
