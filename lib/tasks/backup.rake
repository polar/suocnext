...
namespace :backup do
    ...
    RAILS_APPDIR = Rails.root.sub("/config/..","")
    ...
    def retrive_backup_config
      ...
      backupdir = config[Rails.env] && config[Rails.env][YAML_BACKUPDIR_KEY] ||
                  config[YAML_BACKUPDIR_KEY] || DEFAULT_BACKUPDIR
      archivedir = config[Rails.env] && config[Rails.env][YAML_ARCHIVEDIR_KEY] ||
                   config[YAML_ARCHIVEDIR_KEY] || DEFAULT_ARCHIVEDIR
      sqldumpfile = config[Rails.env] && config[Rails.env][YAML_SQLDUMPFILE_KEY] ||
                    config[YAML_SQLDUMPFILE_KEY] || DEFAULT_SQLDUMPFILE
      dirs        = config[Rails.env] && config[Rails.env][YAML_DIRS_KEY] ||
...
#       backup_config = YAML::load( File.open( 'config/backup.yml' ) )[Rails.env]
#       for server in backup_config["servers"]
#        puts "Backing up #{Rails.env} directorys #{backup_config['dirs'].join(', ')} to #{server['name']}"
...
#    end
    ...
    task :archive do
      ...
      archive_filename = "#{Rails.env}_backup_#{Time.now.strftime("%B.%d.%Y_at_%I.%M.%S%p_%Z")}.tar.gz"
