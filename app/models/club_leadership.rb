...
class ClubLeadership < ActiveRecord::Base
  ...
  attr_accessible :name, :description, :tagline, :position, :activity, :activity_id
  ...
  #
  def leader?(member)
    ClubLeader.where(:leadership_id => self, :member_id => member).first
  end
...
Date.parse(Time.now.to_s)}' <= end_date"
  ...
  def current_active_leaders2
    active_id = ClubMemberStatus.where(:name => 'Active').first.id
    life_id   = ClubMemberStatus.where(:name => 'Life').first.id
