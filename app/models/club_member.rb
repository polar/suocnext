class ClubMember < User
  ...
  validates_date_of :club_start_date, :allow_nil => false
  validates_date_of :club_end_date, :allow_nil => true
  validates_date_of :club_grad_year, :allow_nil => true
  ...
  def self.find_login(username)
    user = ClubMember.where(:login => username).first
    user ||= ClubMember.where(:club_member_id => username.delete(" -")).first
    user ||= ClubMember.where(:email => username).first
    user = nil unless user.activated_at
    user
  end
