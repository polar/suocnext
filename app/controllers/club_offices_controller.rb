...
class ClubOfficesController < BaseController
  ...
  def index
    @club_offices = ClubOffice.all(:order => "position ASC")
  end
  ...
  def new_officer
    ...
    if params[:club_member] && params[:club_member][:login]
      ...
      @selected_officers = ClubMember.where(conditions.join(" AND ")).order("login ASC").limit(AC_CLUB_MEMBER_NAME_LIMIT).all
    end
