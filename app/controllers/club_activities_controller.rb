...
class ClubActivitiesController < BaseController
  ...
  def index
    @club_activities = ClubActivity.order("position ASC").all
  end
  ...
  def new_chair
    ...
    if params[:club_member] && params[:club_member][:login]
      ...
      @selected_chairs = ClubMember.where(conditions.join(" AND ")).order("login ASC").limit(AC_CLUB_MEMBER_NAME_LIMIT).all
    end
