class CertOrgsController < BaseController
  ...
  def auto_complete_for_cert_org_name
    ...
    @items = CertOrg.all(find_options)

    render :inline => "<%= auto_complete_result @items, :name %>"
  end

  def index
    @cert_orgs = CertOrg.all
  end
  ...
  def create
    ...
    if @cert_org.save
      flash[:notice] = "Certification Organization #{@cert_org.name} for type #{@cert_org.cert_type.name} has been created."
      redirect_to(@cert_org)
    else
      flash[:error] = "Certification Organization was not created."
      @submit = "Create"
      @types = CertType.all
      render :action => "new"
    end
  ...
  def update
    ...
    if @cert_org.update_attributes(params[:cert_org])
      flash[:notice] = "Certification Organization #{@cert_org.name} for type #{@cert_org.cert_type.name} has been updated."
      redirect_to(@cert_org)
    else
      flash[:error] = "Certification Organization was not updated."
      @submit = "Update"
      @types = CertType.all
      render :action => "edit"
    end
  ...
  def destroy
    @cert_org = CertOrg.find(params[:id])
    if !CertMemberCert.where(:cert_org_id => @cert_org).all.empty?
      @cert_org.errors.add_to_base "Cannot delete. This Organization is referenced by Members' Certification listings"
      flash[:error] = "Cannot delete Certification Organization #{@cert_org.name} for type #{@cert_org.cert_type.name}." +
                      "It is already referenced by some members."
    else
      @cert_org.destroy
      flash[:notice] = "Certification Organization #{@cert_org.name} for type #{@cert_org.cert_type.name} has been deleted."
    end
