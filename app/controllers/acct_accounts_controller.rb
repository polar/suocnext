class AcctAccountsController < BaseController
  ...
  def index
    ...
    @accounts = AcctAccount.order(:name).paginate(:page => @page, :per_page => @per_page)
  ...
  def show
    ...
    @entries = @account.entries.where("TRUE #{dates}",
                                      :category_id => @category,
                                      :start_date  => @start_date,
                                      :end_date    => @end_date).paginate(:page => @page, :per_page => @per_page)
    @shown_balance = 0
  ...
  def new
    ...
    @actions = AcctAction.order("name ASC").all
  ...
  def destroy
    error = false
    @account = AcctAccount.find(params[:id])
    if !AcctAction.where(:account_id => @account.id).all.empty?
      @account.errors.messages << "Account #{@account.name} has associated Actions."
      error = true
    end
    if !AcctEntry.where(:account_id  => @account).all.empty? ||
       !AcctTransaction.where(:target_account_id => @account).all.empty?
      @account.errors.messages << ("Account #{@account.name} has associated Transactions.")
      error = true
    end
