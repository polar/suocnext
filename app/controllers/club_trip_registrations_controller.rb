class ClubTripRegistrationsController < BaseController
  ...
  def auto_complete_for_club_member_login
    ...
    # AND the queries.
    @items     = ClubMember.where(conditions.join(" AND ")).order("login ASC").limit(AC_CLUB_MEMBER_NAME_LIMIT).all
  ...
  def index
    @club_trip_registrations = ClubTripRegistration.all(
  ...
  def new
    ...
    @leaderships = ClubLeadership.order("name ASC").all
 ...
 trip called #{ctr.trip_name}"
  ...
  def statistics
    ...
    begin
      ...
      @trips      = ClubTripRegistration.includes(:club_members).where('? <= departure_date AND departure_date < ?', start_date1, end_date).all
    rescue ArgumentError
      flash[:error] = "Badly formatted date or date combination"
  ...
  def list_submitted
    ...
    begin
      ...
      @start_date = start_date1.strftime("%m/%d/%Y")
      @end_date   = end_date1.strftime("%m/%d/%Y")
      @trips      = ClubTripRegistration.includes(:club_members).where('? <= departure_date AND departure_date < ? AND submit_date is NOT NULL',
                                                                       start_date1, end_date).order('departure_date DESC').all
    rescue ArgumentError
      flash[:error] = "Badly formatted date or date combination"
      @start_date   = start_date
      @end_date     = end_date
