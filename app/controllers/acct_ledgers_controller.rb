class AcctLedgersController < BaseController
  ...
  def auto_complete_for_club_member_login
    ...
    @items = ClubMember.all(find_options)
  ...
  def new
    ...
    @accounts = AcctAccount.where("account_type_id = ? or account_type_id = ?",
             AcctAccountType[:Asset], AcctAccountType[:Liability]).all
    @submit = "Create Ledger"
  end

  def edit
    @ledger = AcctLedger.find_by_id_or_slug params[:id]
    @accounts = AcctAccount.where(
  ...
  def update_transaction
    ...
    # We always are transferring from the E-Room Account
      ...
      if @transaction.acct_action.name == "Membership Collection"
        if params[:club_membership]
          ...
          if !member
            ...
            member = ClubMember.where(:order => "birthday DESC", :conditions => cond).first
          end
          if member && member.name != name
            @transactions.errors.messages << ("Internal Error: Member '#{name}' does not match the internal id")
            error = true
          end
          if !error && !member
    ...
    if !error && @transaction.save
      if !@membership
        ...
        if @membership.save
          ...
          # Error on saving membership
          @transaction.destroy
          @transaction.errors.messages << ("Cannot save Membership information")
  ...
  def get_transactions_list(targacct, action_set, page, per_page)
    ...
    p conditions
    AcctTransaction.where(conditions).order("date DESC, id DESC").paginate( :page => page, :per_page => per_page)
  end
