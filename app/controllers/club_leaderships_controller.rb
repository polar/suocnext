...
class ClubLeadershipsController < BaseController
  ...
  def index
    @club_leaderships = ClubLeadership.all(:order => "position ASC")
  end
  ...
  def new_leader
    ...
    if params[:club_member] && params[:club_member][:login]
      ...
      @selected_leaders = ClubMember..where(conditions.join(" AND ")).order("login ASC").limit(AC_CLUB_MEMBER_NAME_LIMIT).all
    end
