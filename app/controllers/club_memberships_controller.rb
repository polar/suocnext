class ClubMembershipsController < BaseController
  ...
  def index
    ...
    @memberships   = ClubMembership.includes(:member).where('year = ?', @year).all
  ...
  def render_slackers
    ...
    ms         = ClubMember.includes(:trip_registrations).where(conditions).order('login ASC').all
  ...
  def submit_list
    ...
    ms      = ClubMembership.includes(:member).where('year = ?', year).all
